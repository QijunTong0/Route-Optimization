...
def draw_graph_with_shortest_path(
    ...
    """
    road = expand_graph(
        road, cost=cost_ratio, interp=interp, st_ind=source_node, ed_ind=target_node
    )
    path_nodes = nx.shortest_path(road, source="s", target="t", weight="weight")
    ...
    # )
    nx.draw_networkx_edges(road, pos, width=0.5, edge_color="gray", alpha=1)
    nx.draw_networkx_edges(
        road, pos, edgelist=path_edges, width=2, edge_color="green", alpha=1.0
    )
    ...
    plt.savefig(f"data/anime/out_{suffix:03d}.svg")
    plt.close()
